...
/**
 * This custom formatter for objects of type Product formats objects in the following format.
 * Product name (product GUID), product attribute 1 name:product attribute1 value; ... product attribute n name:product attribute value;
 * Example format : Cool Backpack (111-222-333), maxContentWeight: 15.0, color: "Black", capacity: 20.0
 * Numeric values will retain its type during formatting i.e double will be printed with decimal and integer without decimal
 */
public class ProductStringFormatter implements IProductFormatter<String>{
	...
	public String format(Product product) throws ProductStringFormattingException{
		...
		if(product == null) return formattedOutput;
		
		StringBuffer stringBuffer = new StringBuffer();	
		Formatter formatter = new Formatter(stringBuffer);
		try {
			//propertyMap is created using apache PropertyUtils and contains all properties of the object
			...
			//remove static properties and the class name property prior to formatting dynamic properties.
			propertyMap.remove("name");
			propertyMap.remove("guid");
			propertyMap.remove("class");
			
			//iterate through all other properties available in object and format output using property name and property value
